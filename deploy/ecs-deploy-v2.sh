...
get_latest_tag() {
    ...
    # 尝试拉取 latest 标签，从镜像信息中获取实际标签
    # 或者硬编码为最新的 commit sha
    # 这里从 GitHub 获取最新的 commit sha
...
[ -z "$LATEST" ] && LATEST="sha-44dd5ee"  # 默认值

log "部署版本：$LATEST"
CURRENT=""; [ -f "$STATE_FILE" ] && CURRENT=$(cat "$STATE_FILE" 2>/dev/null)
log "当前：${CURRENT:-无}"
